...
enum MageSpells
{
    ...
    SPELL_MAGE_FIRE_POWER_DAMAGE                 = 83619,
};
...
// 18459 - Fire Power
class spell_mage_fire_power : public SpellScriptLoader
{
public:
    spell_mage_fire_power() : SpellScriptLoader("spell_mage_fire_power") { }

    class spell_mage_fire_power_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_mage_fire_power_AuraScript);

        bool Validate(SpellInfo const* /*spellInfo*/)
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_MAGE_FIRE_POWER_DAMAGE))
                return false;
            return true;
        }
        ...
        void OnRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
        {
            ...
            {
                int8 chance = 33;
                if (GetSpellInfo()->Id == 18460)
                    chance = 66;
                if (roll_chance_i(chance))
                    caster->CastSpell(caster, SPELL_MAGE_FIRE_POWER_DAMAGE, true);
            }
        }

        void Register()
        {
            AfterEffectRemove += AuraEffectRemoveFn(spell_mage_fire_power_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_MOD_DAMAGE_PERCENT_DONE, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_mage_fire_power_AuraScript();
    }
};
